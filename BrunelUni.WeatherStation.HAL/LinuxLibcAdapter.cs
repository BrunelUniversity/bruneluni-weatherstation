...
{
    public class LinuxLibcAdapter : ILinuxLibcAdapter
    {
        ...
        public int Read( int handle, byte [ ] data, int length ) =>
            LibcNativeCommands.ReadNative( handle, data, length );

        public int Write( int handle, byte [ ] data, int length ) =>
            LibcNativeCommands.WriteNative( handle, data, length );
    }
}